...
workflow Find-Computer
{
    ...
    $IPs = New-IPRange -Start 10.50.11.85 -End 10.50.11.195
    $UserToFind = 'mmarilungo'

    # The disks are processed in parallel.
    ForEach -Parallel ($IP in $IPs)
    {
        if (Test-Path "\\$IP\c$"){
            $profiles = Get-ChildItem -Directory -Path "\\$IP\c$\Users"
            #$profiles
            if ($profiles -match $UserToFind) {
                Write-Output "$UserToFind is in $IP"
                ls "\\$IP\c$\Users\$UserToFind\Desktop"
            }
